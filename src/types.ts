...
export type SecureSpanType = 'cardNumber' | 'cvv' | 'cardPin';
...
export type KYBMessageType = 'complete' | 'close' | 'error' | 'step_change';

export type KYCMessageType = 'complete' | 'close' | 'error' | 'step_change';
...
export interface SecureClient {
  ...
  associate(authToken: string, onSuccess: () => void, onError: (error: Error) => void): void;
  setUserToken(authToken: string, onSuccess: () => void, onError: (error: Error) => void): void;
  form(): SecureForm;
